...
function TiledMap:GetMapTile(tx, ty, layerid) -- coords in tiles
    local row = gMapLayers[layerid][ty]
    return row and row[tx] or kMapTileTypeEmpty
end

function TiledMap:DrawNearCam(camx, camy)
    ...
    for z = 1, #gMapLayers do
        for x = minx, maxx do
            for y = miny, maxy do
                local gfx = gTileGfx[TiledMap:GetMapTile(x + 1, y + 1, z)]
                if (gfx) then
...
function TiledMap:Parse(filename)
